#모델 3개 병합한 앙상블 모델
#concatenate에서 Concatenate로 변경
#metrics 추가  -->추가 시 metrics의 loss값도 나옴
import numpy as np
path = '../_data/stock'
x1_datasets = np.array([range(100),range(301,401)]).transpose()
print(x1_datasets)  
x2_datasets = np.array([range(101,201),range(411,511),range(150,250)]).T
print(x2_datasets)  
x3_datasets = np.array([range(100,200),range(1301,1401)]).T

y = np.array(range(2001,2101))  
print(y.shape) #(100,)
y2 = np.array(range(201,301))
print(y2.shape)

from sklearn.model_selection import train_test_split
x1_train, x1_test,x2_train,x2_test,\
    x3_train,x3_test,y_train,y_test,y2_train, y2_test = train_test_split(
    x1_datasets,x2_datasets,\
    x3_datasets,\
    y,y2,train_size=0.7, random_state=1234
)

print(x1_train.shape,x2_train.shape, y_train.shape,y2_train.shape)
print(x1_test.shape,x2_test.shape, y_test.shape,y2_test.shape)

#2.모델 구성

from tensorflow.keras.models import Model
from tensorflow.keras.layers import Dense , Input

#2-1 모델1.
input1 = Input(shape=(2,))
dense1 = Dense(11,activation='relu', name = 'ds11')(input1)
dense2 = Dense(12,activation='relu', name = 'ds12')(dense1)
dense3 = Dense(13,activation='relu', name = 'ds13')(dense2)
output1 = Dense(14,activation='relu', name = 'ds14')(dense3)



#2-2 모델2.
input2 = Input(shape=(3,))
dense21 = Dense(11,activation='linear', name = 'ds21')(input2)
dense22 = Dense(12,activation='linear', name = 'ds22')(dense21)
output2 = Dense(13,activation='linear', name = 'ds23')(dense22)

#2-3 모델3.
input3 = Input(shape=(2,))
dense31 = Dense(11,activation='relu', name = 'ds31')(input3)
dense32 = Dense(12,activation='relu', name = 'ds32')(dense31)
dense33 = Dense(13,activation='relu', name = 'ds33')(dense32)
output3 = Dense(14,activation='relu', name = 'ds34')(dense33)
#2-3 모델 병합
from keras.layers.merge import concatenate, Concatenate
# merge1 = concatenate([output1,output2,output3], name ='mg1')

merge1 = Concatenate()([output1,output2,output3])   #  ***concatenate에서 Concatenate로 변경***
merge2 = Dense(12, activation='relu',name ='mg2')(merge1)
merge3 = Dense(13,name = 'mg3')(merge2)
last_output = Dense(1,name='last')(merge3) #1은 y컬럼 의미

#2-4 모델4 분기1.

dense41 = Dense(11,activation='relu', name = 'ds41')(last_output)
output4 = Dense(14,activation='relu', name = 'ds42')(dense41)

#2-5 모델5 분기2.

dense51 = Dense(11,activation='relu', name = 'ds51')(last_output)
output5 = Dense(14,activation='relu', name = 'ds52')(dense51)



model = Model(inputs = [input1,input2,input3] , outputs = [output4,output5])

model.summary()

#3.컴파일, 훈련
model.compile(loss ='mse',optimizer = 'adam',metrics = ['mae']) #  ***metrics에 대한 loss도 출력됨***
model.fit([x1_train,x2_train,x3_train],[y_train,y2_train],epochs=10, batch_size=32)


#4.평가 , 예측
loss = model.evaluate([x1_test,x2_test,x3_test],[y_test,y2_test])
print('loss : ',loss) # ***loss는 모델의 총합 loss, 나머지는 각각 모델의 loss***